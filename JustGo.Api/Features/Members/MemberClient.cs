    ...
    : JustGoClientBase(httpClient, options), IMemberClient
{
        ...
        FindMembersRequest request, CancellationToken ct)
    {
        ...
        {
            query["CredentialId"] = request.CredentialId.Value.ToString();
        }
        ...
        {
            query["Membership"] = request.Membership;
        }

        if (request.SuspendStatus is not null)
        {
            query["SuspendStatus"] = request.SuspendStatus;
        }

        if (request.ModifiedBefore is not null)
        {
            query["ModifiedBefore"] = request.ModifiedBefore.Value.ToString("O");
        }

        if (request.ModifiedAfter is not null)
        {
            query["ModifiedAfter"] = request.ModifiedAfter.Value.ToString("O");
        }
